...
class SetUp:
    ...
    def getIndexOfMemAddress(cls, currAddr, isSW, dataval, address, numInstructions):
        ...
        try:
            tempIndex = 0
            if isSW:
                # make sure that there are enough spaces in dataval
                # there has to be an even number of elements in dataval

                # if the length of the address is greater than numInstructs
                ...
                tempIndex = tempIndex - numInstructions  # + 1
    ...
    @classmethod
    def isMemOp(cls, index, opcodeStr):
        if opcodeStr[index] == "STUR" or opcodeStr[index] == "LDUR":
            return True
        else:
            return False
