...
class Disassembler:
    ...
    # these lists contain the information for each instruction decoded to enable bringing out the
    # dissasembled file
    opcodeStr = []
    instrSpaced = []
    arg1 = []
    ...
    def run(self):
        ...
        for i in range(len(opcode)):
            ...
            if opcode[i] == 1112:
                ...
                self.arg2.append((int(instructions[i], base=2) & self.addr2Mask) >> 5)
                self.arg3.append((int(instructions[i], base=2) & self.rdMask) >> 0)
                self.arg1Str.append("\tR" + str(self.arg3[i]))
                ...
                self.arg1.append(SetUp.immSignedToTwosConverter((int(instructions[i], base=2) & self.addr2Mask) >> 5))
